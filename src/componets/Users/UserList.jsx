...
const Userlist = () => {
  ...
  const getUsers = async () => {
    try {
      const response = await axios.get('https://fabric-ternak-backend.my.to/users');
  ...
  const deleteUser = async (userId) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          axios.delete(`https://fabric-ternak-backend.my.to/users/${userId}`).then(() => {
            getUsers();
            Swal.fire({
              title: "Terhapus!",
  ...
  return (
    <div className="relative overflow-x-auto shadow-md sm:rounded-lg">
      <h1 className='font-bold grid justify-items-center text-3xl mt-6 mb-6'> DATA PENGGUNA </h1>
        ...
        <tbody>
          {filteredUsers.map((user, index) => (
            <tr key={user.uuid} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
              ...
              <td className="px-6 py-4">
                <Link to={`/users/edit/${user.uuid}`} className="focus:outline-none text-white bg-yellow-400 hover:bg-yellow-500 focus:ring-4 focus:ring-yellow-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:focus:ring-yellow-900">Edit</Link>
                <button onClick={() => deleteUser(user.uuid)} type="button" className="focus:outline-none text-white bg-red-700 hover:bg-red-800 focus:ring-4 focus:ring-red-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-900">Delete</button>
              </td>
            </tr>
          ))}
        </tbody>
