...
const PembaruanSapiList = () => {
  ...
  const filteredSapi = sapi.filter((sapi) =>
    ...
    sapi.jenisSapi.toLowerCase().includes(searchQuery.toLowerCase())
      ...
      .filter((sapi) => 
      sapi.umurSekarang === null && sapi.konfirmasiVaksinasi !== 'Belum Dikonfirmasi'
  );

  return (
    <div className="relative overflow-x-auto shadow-md sm:rounded-lg">
      <h1 className='font-bold grid justify-items-center text-3xl mt-6 mb-7'>PEMBARUAN DATA SAPI </h1>
      ...
      <table className="w-full min-w-[100%] text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
        ...
        <span className={`truncate px-2 py-1 ${sapi.konfirmasiVaksinasi === 'Belum Dikonfirmasi' ? 'bg-red-700' : 'bg-green-700'} text-white rounded`}>
              ...
              <Link to={`/pembaruan/update/${sapi.uuid}`}>
                <button
                  type="button"
                  className={`focus:outline-none text-white bg-yellow-400 hover:bg-yellow-500 focus:ring-4 focus:ring-yellow-300 font-medium rounded-lg text-sm px-5 py-2.5 dark:focus:ring-yellow-900 ${
                    sapi.beratSekarang && sapi.umurSekarang ? 'opacity-50 cursor-not-allowed' : ''
                  }`}
                  disabled={sapi.beratSekarang && sapi.umurSekarang}
                >
                  Perbarui
                </button>
              </Link>
...
export default PembaruanSapiList;
